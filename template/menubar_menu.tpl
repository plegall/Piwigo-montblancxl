...
{/if}
	...
	{foreach from=$block->data item=link}
		{if is_array($link)}
			<li><a href="{$link.URL}" title="{$link.TITLE|default:''}"{if isset($link.REL)} {$link.REL}{/if}>{$link.NAME}</a>{if isset($link.COUNTER)}<span class="menuInfoCat">({$link.COUNTER})</span>{/if}</li>
		{/if}
	{/foreach}
	{/strip}</ul>
</dd>
